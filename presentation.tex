...
\begin{frame}{Circumvent Quantum Errors}
	\begin{center}
        \includegraphics[width=0.6\textwidth]{fig/TypesErrorCorrection.png}
    \end{center}
	\begin{itemize}
...
\begin{frame}{QC development}
	...
	\begin{overlayarea}{\textwidth}{3cm}
		...
		\hspace{3cm}
		\begin{tikzpicture}
			\draw[->,  thick, osakared] (0cm, 0cm) -- (0.8cm, 0cm);
			\draw[->, thick, osakared] (6cm, 0cm) -- (6.8cm, 0cm);
		\end{tikzpicture}
	\end{overlayarea}
...
\begin{frame}{Surface Code Suboptimal Decoders}
	...
	\pause
	\only<4-4>{
	\begin{textblock*}{6cm}(0.1cm,2cm)
		\begin{small}
			\textbf{Minimum Weight Perfect Matching}
			\begin{itemize}
				\item Outputs always errors which match the syndrome
				\item Improved: Sparse blossom $O(N^{1.32})$, fusion blossom $O(N^{1.32})$
				\item Sparse blossom real time operational for $0.1\%$ depolarising noise on distance-17 surface code \cite{higgott_sparse_2025}
				\item Noise aware \cite{iolius_performance_2022}
			\end{itemize}
		\end{small}
	\end{textblock*}
	\begin{textblock*}{10cm}(8.7cm,3.5cm)  % Adjust position as needed
		\begin{tikzpicture}
	...
	\begin{textblock*}{6cm}(0.1cm,3cm)
		\begin{small}
			...
			\begin{itemize}
				...
				\item Approximate contraction by truncation on SVD
				\item Generalised to arbitrary 2D codes \cite{chubb_general_2021}
			\end{itemize}
		\end{small}
	\end{textblock*}
	}
	% 	TN Decoder & \cite{bravyi_efficient_2014}

\end{frame}

% experiments of logical comp and mem below and above threshold
...
% Correlated Noise
% CFT at Nishimori point RG flow along CFTs when disturbign the noise model continuously
...
\begin{frame}{Research plan}
	...
	\end{itemize}
	% We developed a hardware adaptive simulation framework to estimate logical quantum memory performance. \\
	% The framework is easily adapted to varying error correcting codes and noise models.\\
	% Applied it to the toric code under bitflip noise - which acts in the field of QEC as the pendulum in robotics.\\
	% In a next step QuaSA offers the possibility to deduce actual realistic noise models by experimental data. \\
	% Subsequently it is planned to extend the simulation results to these realistic noise models and may improve the simulation framework by hardware information. \\
\end{frame}
