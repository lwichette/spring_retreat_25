...
\begin{frame}{Error Classes}
	\only<1-1>{
		\begin{textblock*}{15cm}(0.5cm,2cm)
			\begin{itemize}
				\item $\mathcal{S} \subset \mathcal{P}_n$ the stabilizer group of a code (Abelian subgroup of $n$-qubit Pauli group).
				\item Error $E$ maps a code state $|\psi\rangle$ to $E|\psi\rangle$.
				\item $E_1$ and $E_2$ varying by stabilizer:
				\[
					E_1 = E_2 S \quad \text{for some } S \in \mathcal{S}
				\]
				Then,
				\[
					E_1|\psi\rangle = E_2 S |\psi\rangle = E_2 |\psi\rangle
				\]
				since $S|\psi\rangle = |\psi\rangle$ for all $|\psi\rangle$ in the code space.
				\item \textbf{Conclusion:} $E_1$ and $E_2$ have the same effect on the code space.
			\end{itemize}
		\end{textblock*}
	}
	\only<2-2>{
		\begin{textblock*}{15cm}(0.5cm,2cm)
			\begin{itemize}
				\item The \textbf{syndrome} depends only on commutation with stabilizers:
				\[
					\text{syndrome}(E) = \{ S_i E = \pm E S_i \}
				\]
				If $E' = E S$, then $E'$ has the same syndrome as $E$, since stabilizers commute with each other:
				\[
					[S_i, E'] = [S_i, E S] = [S_i, E] S + E [S_i, S] = [S_i, E] S
				\]
			\end{itemize}
			\vspace{1em}
			\textbf{Key Idea:} Errors differing by a stabilizer produce the same syndrome and act identically on the code—so we group them into equivalence classes.
		\end{textblock*}
	}
	\only<3->{
		\vspace{-2cm}
		Toric code under bit-flip noise:
...
\begin{frame}{Statistical Mechanics Mapping}
	\only<1-1>{
		\begin{textblock*}{14cm}(0.5cm,2cm)
			Error correcting code with error $e$ $\rightarrow$ statistical mechanics system with Hamiltonian $H_e$:
			\begin{itemize}
				\item \textbf{Stabilizer:} $S_k$ $\rightarrow$ classical spin dof $s_k \in \{-1,1\}$
				\item \textbf{Qubits:} qubit $i$ with local Pauli group operator $\sigma$ defines interaction $J_{i}(\sigma)$ between classical spin dof $s_k$ satisfying:\\
				\item \textbf{Hamiltonian:} $H_e(\{s_i\}) = -\sum_{i, \sigma \in \mathcal{P}_{i}}J_{i}(\sigma) [\sigma, e]_{\text{scalar}} \prod_{k:[\sigma, S_k]_{\text{scalar}}= -1} \sigma_k$
				\item \textbf{Nishimori condition:} $\beta J_i(\sigma) = \frac{1}{\|\mathcal{P}\|}\sum_{\tau \in \mathcal{P}_i}\log p_i(\tau)[\sigma, \tau^{-1}]_{\text{scalar}} \forall i, \sigma \in \mathcal{P}_i$
				\item \textbf{Boltzmann weight:} $P(e) = \exp(-\beta H_e(\{\Uparrow \}))$
				\item \textbf{Partition function:} $P([e]) = Z(e)=\sum_{\{\sigma_k\}} \exp(-\beta H_e(\{\sigma_k\}))$
			\end{itemize}
		\end{textblock*}
	}
	\only<2->{
		...
		\begin{textblock*}{6cm}(0.5cm,3cm)
			\begin{center}
				\includegraphics[width=1\textwidth]{fig/BU_StatMech_1.png}
			\end{center}
		\end{textblock*}
		\begin{textblock*}{6cm}(7.5cm,3cm)
			\begin{center}
				\includegraphics[width=1\textwidth]{fig/BU_StatMech_3.png}
			\end{center}
		\end{textblock*}
	}
\end{frame}
...
\begin{frame}{Fisher Kasteleyn Temperley Algorithm}
	\only<1-1>{
		\begin{textblock*}{9cm}(0cm,2cm)
		\begin{itemize}
			\item  The toric code with bit-flip noise maps to a 2D RBIM:
			$
			H_{\text{RBIM}} = -\sum_{\langle ij \rangle} J_{ij} S_i S_j
			$
			\item Spins \( S_i = \pm 1 \) live on faces of the dual lattice \( G^\star \).
			\item Interactions \( J_{ij} \) are random: ferromagnetic/antiferromagnetic bonds encode noise.
			\item Domain walls separate regions of flipped spins: edges where \( S_i S_j = -1 \).
			\item Relative domain walls correspond to configurations up to a global spin flip.
			\item $P([e]) = Z(e)=\sum_{\{\sigma_k\}} \exp(-\beta H_e(\{\sigma_k\}))$
		\end{itemize}
		\end{textblock*}
		\begin{textblock*}{5cm}(9.5cm,3cm)
			\begin{center}
	...
	\only<2-2>{
		\begin{textblock*}{9cm}(0cm,2cm)
		\begin{itemize}
			\item \textbf{Kasteleyn cities}: Spin configurations map to \textit{perfect matchings} (dimers) on decorated \( \tilde{G}^\star \), though not bijectively.
	...
	\only<3-3>{
		...
		\begin{itemize}
			\item $PF(K)=\frac{1}{n!2^{n}}\sum_{\pi\in S_{2n}}\sigma(\pi)\prod_{j=1}^{n}K_{\pi(2j-1), \pi(2j)}$
			\item For planar graphs: $Z=Pf(K_{\text{Pf}})$
			\item Torus: More complicated as not all perfect matchings are associated with physical states \\
			$2Z=Pf(K^{++})+Pf(K^{+-})+Pf(K^{-+})+Pf(K^{--})$
		\end{itemize}
		\end{textblock*}
		\begin{textblock*}{5cm}(9.5cm,1.5cm)
			\begin{center}
...
\begin{frame}{Wang Landau Algorithm: Motivation}
	...
	\item \textbf{Key Idea:} Sampling from system with $g(E_i)$ with acceptance ratio $\rho(E_i)\sim\frac{1}{g(E_i)}$,\\
...
% 			2014 & Nigg et al. & Seven-qubit color code on trapped-ion device and execution of logical single-qubit gates \citeauthoryear{nigg_quantum_2014} \\
% 			2015 & Kelly et al. &  Nine qubit repetition code on superconducting processor \citeauthoryear{kelly_state_2015} \\
% 			2020 & Erhard et al. & Lattice surgery generated entanglement of two logical qubits on a ten qubit ion trap device \citeauthoryear{erhard_entangling_2020} \\
% 			2021 & Ryan et al. & Real time decoding on seven-qubit color code on trapped-ion device and non-Clifford operation \citeauthoryear{ryan-anderson_realization_2021}\\
% 			2023-24 & Google Quantum AI et al. & Real time decoding and below threshold quantum memory experiments on up to 101 qubits on superconducting processor \citeauthoryear{google_quantum_ai_suppressing_2023, google_quantum_ai_and_collaborators_quantum_2025}
% 		\end{tabular}
% 	\end{table}
% \end{frame}
...
% 			\hline
% 			Toric Code \citeauthoryear{hirota_quantum_1997} & $2d^{2}$ & 2 & d \\
% 			Planar Surface Code & $d^{2} + (L-1)^{2}$ &  1 & d \\
% 			Rotated Surface Code & $d^{2}$ &  1 & d \\
% 			\hline
% 			\pause
% 			Honneycomb Code \citeauthoryear{landahl_fault-tolerant_2011} & $\frac{3}{4}d^{2}$ + $\frac{1}{4}$ & 1 & d \\
% 			\hline
% 			\pause
% 			Quantum Tanner Code \citeauthoryear{leverrier_quantum_2022} & n & $\Omega(n)$ & $\Omega(n)$
% 		\end{tabular}
% 	\end{table}
% \end{frame}
...
\begin{frame}{Tackling Quantum Errors}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \textbf{Passive:} Store quantum information in a self correcting way \citeauthoryear{bacon_operator_2006, berthusen_experiments_2024}
		\end{itemize}
	\end{itemize}
\end{frame}
