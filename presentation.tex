...
\begin{frame}{Overview Literature Research}
	\begin{center}
		\begin{itemize}
			\only<1->{
			\item Brief history of QEC
			}
			\vspace{0.5cm}
			\pause
			\only<2->{
			\item Codes and teir quality markers
			\vspace{0.5cm}
			\item Decoder and their quality markers
			}
			\vspace{0.5cm}
			\pause
			\only<3->{
			\item Experimental realization
			}
			\vspace{0.5cm}
			\pause
			\only<4->{
			\item Methods to estimate optimal code performance
			}
		\end{itemize}
	\end{center}
\end{frame}
...
\begin{frame}{Research goal}
	...
	\begin{itemize}
		...
		\item Investigate RG flow of CFTs at Nishimori point by disturbing noise model
	\end{itemize}
\end{frame}
...
\begin{frame}{Intermezzo - Groundwork for Results}
	...
	\only<7-7>{
		...
		\begin{textblock*}{8cm}(8cm,2cm)
			...
			$\delta(s) = \arg\max_{l\in L}P([C_{s}L_{l}])$\\
			\vspace{0.8cm}
			\textbf{Z decoding} by statistical mechanics mapping \cite{dennis_topological_2002}:\\
			$\delta(s) = \arg\max_{l\in L}Z^{T_{\text{Nishimori}}}(C_{s}L_{l})$\\
		\end{textblock*}
	}
	\only<8-8>{
		\begin{textblock*}{16cm}(2cm,1.7cm)
		...
		\end{textblock*}
		\begin{textblock*}{8cm}(8cm,2cm)
			\textbf{Z decoding at temperature T}\\
			$C_{s} \in \mathcal{G}_{n}$ compatible with $s$\\
			$[C_{s}]=SC_{s}$ \\
			...
			$\bar{\delta(s)}$ with $P(\bar{\delta(s)} = l\in L)=\frac{Z^{T}(C_{s}L_{l})}{\sum_{l\in L}Z^{T}(C_{s}L_{l})}$
		\end{textblock*}
	}
	\only<9-9>{
		\begin{textblock*}{16cm}(2cm,1.7cm)
		...
		\begin{textblock*}{8cm}(8cm,2cm)
			...
			$\bar{\delta(s)}$ with $P(\bar{\delta(s)} = l\in L)=\frac{Z^{T}(C_{s}L_{l})}{\sum_{l\in L}Z^{T}(C_{s}L_{l})}$\\
			\vspace{0.8cm}
			Estimation of Z by REWL and FKT
		\end{textblock*}
	}
	% \only<6-6>{
	% \begin{textblock*}{16cm}(4cm,1.7cm)
...
\begin{frame}{Preliminary Results}
	...
	\only<2->{
		...
		\hspace{1.5cm}
		\begin{itemize}
			\item Z decoding $\sim$ decoding probability
			\item Probabilistic Z decoding $\sim$ order probability
			\item Decoding probability and order probability require less samples for same accuracy than respective Z decoding
			% \item At low temperature: Order probability $\sim$ maximum probability decoding and decoding probability $\sim$ degeneracy enhanced maximum probability decoding
			% \item Enhancing MWPM by takign degeneracy into account can approximate decoding probability
