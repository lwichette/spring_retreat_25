...
\begin{frame}{Intermezzo for technicalities}
	\only<1-1>{
	...
	\only<7-7>{
		...
		\begin{textblock*}{8cm}(8cm,2cm)
			...
			$\delta(s) = \arg\max_{l\in L}P([C_{s}L_{l}])$\\
			\vspace{0.8cm}
			Statistical mechanics mapping \cite{dennis_topological_2002}:
			$\delta(s) = \arg\max_{l\in L}Z_{C_{s}L_{l}}(\beta_{\text{Nishimori}})$\\
			\vspace{0.8cm}
			Estimation of Z by REWL and FKT for varying temperatures and physical error rates
		\end{textblock*}
	}
	\only<8-8>{
		Define further:
