...
\begin{frame}{Suboptimal Code Performance Estimates}
	\only<1-1>{
	...
	\begin{textblock*}{8cm}(0.5cm,4cm)
		...
		Failure curves estimated as quality measure for various codes at finite sizes under varying noise models~\citeauthoryear{demarti_iolius_decoding_2024}
	\end{textblock*}
	}
\end{frame}
...
\begin{frame}{Optimal Code Performance}
	...
	\begin{textblock*}{8cm}(0.5cm,3cm)
		\begin{itemize}
			...
			\item statistical mechanics mapping~\citeauthoryear{dennis_topological_2002}: \\
				Relates threshold to a phase transition of associated physical system $\rightarrow$ Makes threshold calculation feasible through established methods
		\end{itemize}
	\end{textblock*}
	}
	\only<2-2>{
	...
	\begin{textblock*}{8cm}(0.5cm,2.5cm)
		The statistical mechanics mapping has been extensively used to calculate the threshold of topological codes:
		\pause
		\begin{itemize}
...
\begin{frame}{Overview Results}
	\begin{itemize}
		...
		\item Compared maximum probability and maximum likelihood decoding
...
\begin{frame}{Groundwork for Results - Success Rates}
	...
	\only<2-2>{
		\vspace{0.5cm}
		The order probability~\citeauthoryear{thomas_simplest_2011} is a temperature dependent parameter which was introduced for the investigation of phase transitions in statistical mechanics.
	}
	\only<3->{
		\vspace{0.5cm}
		\textbf{Maximum Probability/Minimum Weight decoding:}\\
		Succees rate estimator: Order probability in zero temperature limit\\
	}
	\only<4->{
		\vspace{0.5cm}
		\textbf{Maximum likelihood/Optimal decoding:}\\
...
\begin{frame}{Results - Zero T Limit}
	...
	\vspace{0.5cm}
	\textbf{Numerically:} \\
	Replica exchange Wang Landau algorithm~\citeauthoryear{vogel_generic_2013, wang_efficient_2001} - can take limit but slow\\
	Fischer Kasteleyn Temperley algorithm~\citeauthoryear{kasteleyn_statistics_1961, temperley_dimer_1961, thomas_exact_2009, thomas_numerically_2013} - only finite T but fast\\
	\pause
	\begin{tcolorbox}[colback=osakared!5!white, colframe=osakared, width=13cm, arc=2mm]
